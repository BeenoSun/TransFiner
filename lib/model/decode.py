...
from post_transfiner.utils.utils import track_merge
...
def generic_decode(output, K=100, opt=None, gt_box=None, cnt=None, area=None, num_re=None, tes=False):
  if not ('hm' in output):
    return {}
  ...
  flag = False
  if 'result_refine' in output:
    patch_area = output['patch_bbox']

    patch_area = (patch_area.detach().clone() *
                  torch.tensor([opt.input_w//4, opt.input_h//4, opt.input_w//4, opt.input_h//4], device=patch_area.device)[None]).int()

    ret, ret_ori = track_merge(patch_area, ret, output, gt_box=gt_box)
