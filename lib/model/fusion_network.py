...
class REDNet20(nn.Module):
    def __init__(self, num_layers=10, num_features=64):
        ...
        self.aggre = nn.Sequential(nn.Conv2d(3, 1, kernel_size=1, stride=1, padding=0),
                                   nn.BatchNorm2d(1),
                                   nn.ReLU(inplace=True))
...
class REDNet30(nn.Module):
    def __init__(self, num_layers=15, num_features=64):
        ...
        self.deconv_layers = nn.Sequential(*deconv_layers)
        self.relu = nn.ReLU(inplace=True)
        self.aggre = nn.Sequential(nn.Conv2d(3, num_features, kernel_size=1, stride=1, padding=0),
                                   nn.BatchNorm2d(num_features),
                                   nn.ReLU(inplace=True),
                                   nn.Conv2d(num_features, 1, kernel_size=1, stride=1, padding=0))
        self.sigmoid = nn.Sigmoid()
