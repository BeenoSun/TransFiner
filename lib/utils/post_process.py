...
def generic_post_process(
  opt, dets, c, s, h, w, num_classes, calibs=None, height=-1, width=-1, potential=False, ori=False):
  ...
  for i in range(len(dets['scores'])):
    preds = []
    trans = get_affine_transform(c[i], s[i], 0, (w, h), inv=1).astype(np.float32) if not ori else get_affine_transform(c[i], s[i], 0, (240, 136), inv=1).astype(np.float32)
    for j in range(len(dets['scores'][i])):
      if dets['scores'][i][j] < (opt.out_thresh if not potential else 0.01):
        continue
      item = {}
