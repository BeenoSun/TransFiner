...
class Tracker(object):
  ...
  def step(self, results, public_det=None, ori=True):
    N = len(results)
    M = len(self.tracks)
    if ori:
      dets = np.array(
      ...
      # refinement matching
      det_box = torch.stack([torch.tensor(obj['bbox']) for obj in results], dim=0) if len(results) != 0 else \
        torch.ones((0, 4))  # N x 4  # N x 4
      track_box = torch.stack([torch.tensor(obj['bbox']) for obj in self.tracks], dim=0) if len(self.tracks) != 0 else \
        torch.ones((0, 4), device=det_box.device)  # M x 4
      cost_bbox = 1.0 - box_ops.generalized_box_iou(det_box, track_box) if len(self.tracks) != 0 else \
        torch.ones((N, M), device=det_box.device)
    else:
      dets = np.array(
        [det['tracking'] for det in results], np.float32)  # N x 2
      track_size = np.array([((track['bbox'][2] - track['bbox'][0]) * \
                              (track['bbox'][3] - track['bbox'][1])) \
                             for track in self.tracks], np.float32)  # M
      track_cat = np.array([track['class'] for track in self.tracks], np.int32)  # M
      ...
      item_cat = np.array([item['class'] for item in results], np.int32)  # N
      tracks = np.array(
        [pre_det['bbox'] for pre_det in self.tracks], np.float32)  # M x 2
      dist = (((tracks.reshape(1, -1, 4) - \
                dets.reshape(-1, 1, 4)) ** 2).sum(axis=2))/2  # N x M
      ...
      # refinement matching
      det_box = torch.from_numpy(dets) if len(results) != 0 else \
        torch.ones((0, 4))  # N x 4  # N x 4
      track_box = torch.stack([torch.tensor(obj['bbox']) for obj in self.tracks], dim=0) if len(self.tracks) != 0 else \
        torch.ones((0, 4))  # M x 4
      cost_bbox = 1.0 - box_ops.generalized_box_iou(det_box, track_box) if len(self.tracks) != 0 else \
        torch.ones((N, M))


    #print('using enhanced tracking!')

    if self.opt.hungarian:
      item_score = np.array([item['score'] for item in results], np.float32) # N
      dist[dist > 1e18] = 1e18
      matched_indices = linear_assignment(dist) if ori else linear_assignment(cost_bbox)
    else:
    ...
    if self.opt.hungarian:
      ...
      for m in matched_indices:
        if (dist[m[0], m[1]] if ori else cost_bbox[m[0], m[1]]) > (1e16 if ori else 1.2):
