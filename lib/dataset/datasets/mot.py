...
class MOT(GenericDataset):
    num_categories = 1
    default_resolution = [544, 960]#[640, 896]
    class_name = ['']
    max_objs = 256
    cat_ids = {1: 1, -1: -1}
    ...
    def __init__(self, opt, split):
        ...
        # addition
        img_dir = os.path.join(data_dir, '{}'.format('test'))
        ann_file = '{}.json'.format('test')
    ...
    def run_eval(self, results, save_dir):
        ...
        # addition
        gt_type_str = '_'+opt.dataty_debug+'_half'

        gt_type_str = '--gt_type {}'.format(gt_type_str) if gt_type_str != '' else \
            ''

        os.system('python tools/TrackEval/run_mot_challenge.py ' +
                  '--BENCHMARK MOT17 --SPLIT_TO_EVAL train --TRACKERS_TO_EVAL /home/beeno/pycharm/py_code/CenterTrack/exp/tracking/mot17_half/results_mot17halfval \
        --METRICS HOTA CLEAR Identity --USE_PARALLEL False --NUM_PARALLEL_CORES 1')
