...
class GenericDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    if opt.tracking:
      ...
      pre_cts, track_ids = self._get_pre_dets(
        pre_anns, trans_input_pre, trans_output_pre, hm_plot=False, other_clues=True)
      ret['pre_img'] = pre_img
      ...
      if opt.same_aug_pre and frame_dist != 0:
        ...
        trans_output_pre2 = get_affine_transform(
          c_pre, s_pre, rot, [opt.output_w, opt.output_h])
      pre2_img = self._get_input(pre2_image, trans_input_pre2)
  ...
  def _get_pre_dets(self, anns, trans_input, trans_output, hm_plot, other_clues):
    down_ratio = self.opt.down_ratio
    hm_h, hm_w = int(self.opt.input_h / down_ratio), int(self.opt.input_w / down_ratio)
    trans = trans_output
