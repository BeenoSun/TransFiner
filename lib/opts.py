...
class opts(object):
    def __init__(self):
        ...
        self.parser.add_argument('--load_track_model', default='',
                                 help='path to pretrained model')
        ...
        self.parser.add_argument('--ori_input_h', type=int, default=544,
                                 help='input height. -1 for default from dataset.')
        self.parser.add_argument('--ori_input_w', type=int, default=960,
                                 help='input width. -1 for default from dataset.')
        ...
        self.parser.add_argument('--lr_step_weight', type=str, default='1',
                                 help='drop learning rate by 10.')
        ...
        self.parser.add_argument('--refine_thresh', type=float, default=-1,
                                 help='')
        self.parser.add_argument('--patch_thresh', type=float, default=-1,
                                 help='')
        ...
        self.parser.add_argument('--new_thresh', type=float, default=0.45) # addition
        ...
        self.parser.add_argument('--hungarian', action='store_false') # addition
        self.parser.add_argument('--max_age', type=int, default=32) # addition
        ...
        # new
        #   magic box part
        self.parser.add_argument('--x_weight', type=float, default=1.)
        self.parser.add_argument('--y_weight', type=float, default=3.)
        self.parser.add_argument('--z_weight', type=float, default=15.)
        self.parser.add_argument('--offset_denomin', type=float, default=1.)
        self.parser.add_argument('--wh_denomin', type=float, default=10)
        self.parser.add_argument('--tracking_denomin', type=float, default=1.)
        #   BiConvLSTM part
        self.parser.add_argument('--biconvlstm_kernel_size', type=int, default=3)
        self.parser.add_argument('--biconvlstm_hidden_dim', type=int, default=1)
        self.parser.add_argument('--biconvlstm_input_dim', type=int, default=3)
        self.parser.add_argument('--stack_num', type=int, default=8)
        self.parser.add_argument('--biconvlstm_num_layers', type=int, default=1)
        self.parser.add_argument('--loss_second_alpha', type=float, default=2.)
        self.parser.add_argument('--loss_second_beta', type=float, default=1.1)
        self.parser.add_argument('--hm_lift', type=float, default=0.0004)
        #   Gradient accumulation
        self.parser.add_argument('--accum_iter', type=int, default=1)
        #   Heatmap refine
        self.parser.add_argument('--bias', type=float, default=1.25)
        self.parser.add_argument('--weight', type=float, default=0.95)
        self.parser.add_argument('--w_pr', type=float, default=0.1)
        self.parser.add_argument('--w_corr', type=float, default=1.)
        self.parser.add_argument('--w_refinemap', type=float, default=0.03)
        #   amp
        self.parser.add_argument('--withamp', action='store_true')
        #   improved focus loss
        self.parser.add_argument('--stretch_factor', type=float, default=0.9)
        self.parser.add_argument('--alert_fn', type=float, default=.6)
        self.parser.add_argument('--train_thres', type=float, default=.4)
        self.parser.add_argument('--divide', type=float, default=0)

        #   dataty
        self.parser.add_argument('--dataty', default='train')
        self.parser.add_argument('--dataty_debug', default='train')
        self.parser.add_argument('--crowdma_path', default='')

        # * Backbone
        self.parser.add_argument('--backbone_refine', default='resnet50', type=str,
        ...
        # * transpatch (tnt + n_p_q patch queries)
        self.parser.add_argument('--n_p_q', type=int, default=64)
        self.parser.add_argument('--depth', type=int, default=10)
        self.parser.add_argument('--aux_loss_tnt', default=True, action='store_false')
        self.parser.add_argument('--num_patch_val', type=int, default=3)
        self.parser.add_argument('--transpatch_trainonly', default=False, action='store_true')
        self.parser.add_argument('--transformer_trainonly', default=False, action='store_true')
        self.parser.add_argument('--test_transpatch', default=False, action='store_true')
        self.parser.add_argument('--eva', default=False, action='store_true')


        # * model setting
        self.parser.add_argument("--det_token_num", default=100, type=int,
                            help="Number of det token in the deit backbone")
        self.parser.add_argument('--backbone_name', default='small', type=str,
                            help="Name of the deit backbone to use")
        self.parser.add_argument('--pre_trained', default='',
                            help="set imagenet pretrained model path if not train yolos from scatch")
        self.parser.add_argument('--init_pe_size', nargs='+', type=int,
                            help="init pe size (h,w)")
        self.parser.add_argument('--mid_pe_size', nargs='+', type=int,
                            help="mid pe size (h,w)")
        self.parser.add_argument('--use_checkpoint', default=False, action='store_true')
        ...
        self.parser.add_argument('--num_queries', default=500, type=int,
                            help="Number of query slots")
        self.parser.add_argument('--real_num_queries', default=150, type=int,
                                 help="Number of query slots")
        self.parser.add_argument('--dec_n_points', default=4, type=int)
        self.parser.add_argument('--enc_n_points', default=4, type=int)
        self.parser.add_argument('--box_amodal_pred', default=True, action='store_false')
        self.parser.add_argument('--nms_thre', default=0.7, type=float,
                                 help="nms used in post processor filtering the real_num_queries")
        self.parser.add_argument('--nms_thre_nd', default=0.7, type=float,
                                 help="nms used in post refine filtering the remaining above!")
        self.parser.add_argument('--reconstruct_thre', default=0.3, type=float,
                                 help="nms used in post refine filtering the remaining above!")
        self.parser.add_argument('--throw_baseline', default=0.4, type=float,
                                 help="nms used in post processor filtering the real_num_queries")
        self.parser.add_argument('--duplicate_baseline', default=0.05, type=float,
                                 help="nms used in post processor filtering the real_num_queries")
        self.parser.add_argument('--noise', default=0.1, type=float,
                                 help="nms used in post processor filtering the real_num_queries")


        # * Loss coefficients
        self.parser.add_argument('--mask_loss_coef', default=1, type=float)
        self.parser.add_argument('--dice_loss_coef', default=1, type=float)
        self.parser.add_argument('--cls_loss_coef', default=2, type=float)
        self.parser.add_argument('--bbox_loss_coef', default=5, type=float)
        self.parser.add_argument('--giou_loss_coef', default=2, type=float)
        self.parser.add_argument('--pre_bbox_loss_coef', default=1, type=float)
        self.parser.add_argument('--pre_giou_loss_coef', default=0.4, type=float)
        self.parser.add_argument('--p_cls_loss_coef', default=2, type=float)
        self.parser.add_argument('--p_bbox_loss_coef', default=3.5, type=float)
        self.parser.add_argument('--p_giou_loss_coef', default=3.5, type=float)
        self.parser.add_argument('--whr_norm_coef', default=0.15, type=float)
        self.parser.add_argument('--focal_alpha', default=0.25, type=float)
        self.parser.add_argument('--patch_focal_alpha', default=0.15, type=float)# 0.25
        self.parser.add_argument('--id_loss_coef', default=1, type=float)

        # Loss
        self.parser.add_argument('--no_aux_loss', dest='aux_loss', action='store_false',
                            help="Disables auxiliary decoding losses (loss at each layer)")

        # Variants of Deformable DETR
        self.parser.add_argument('--with_box_refine', default=True, action='store_true')
        self.parser.add_argument('--two_stage', default=False, action='store_true')

        self.parser.add_argument('--lr_backbone_names', default=["backbone.0"], type=str, nargs='+')
        self.parser.add_argument('--lr_backbone', default=2e-5, type=float)
        self.parser.add_argument('--lr_linear_proj_names', default=['reference_points', 'sampling_offsets'], type=str,
                            nargs='+')
        ...
        self.parser.add_argument('--hm_prob_init', default=0.009, type=float)

        # * Segmentation
        self.parser.add_argument('--masks', action='store_true',
                            help="Train segmentation head if the flag is provided")

        self.parser.add_argument('--checkpoint_enc_ffn', default=False, action='store_true')
        self.parser.add_argument('--checkpoint_dec_ffn', default=False, action='store_true')

        # * Matcher
        self.parser.add_argument('--set_cost_class', default=2, type=float,
                            help="Class coefficient in the matching cost")
        self.parser.add_argument('--set_cost_bbox', default=5, type=float,
                            help="L1 box coefficient in the matching cost")
        self.parser.add_argument('--set_cost_giou', default=2, type=float,
                            help="giou box coefficient in the matching cost")
        self.parser.add_argument('--set_p_cost_class', default=2, type=float,
                                 help="Class coefficient in the matching cost")
        self.parser.add_argument('--set_p_cost_bbox', default=3.5, type=float,
                                 help="L1 box coefficient in the matching cost")
        self.parser.add_argument('--set_p_cost_giou', default=3.5, type=float,
                                 help="giou box coefficient in the matching cost")
    ...
    def parse(self, args=''):
        ...
        opt.lr_step_weight = [float(i) for i in opt.lr_step_weight.split(',')]
        ...
        if 'tracking' in opt.task:
            ...
            opt.tracking = True
            opt.out_thresh = max(opt.track_thresh, opt.out_thresh) # track 0.4
            opt.pre_thresh = max(opt.track_thresh, opt.pre_thresh) # pre 0.5
            opt.new_thresh = max(opt.track_thresh, opt.new_thresh) # new 0.4
        ...
        opt.save_dir = os.path.join(opt.exp_dir, opt.exp_id) # , 'test'  remove 'test' if train
