...
class Detector(object):
  def __init__(self, opt):
    ...
    self.model, self.model_second = create_model(
  ...
  def process(self, images, pre_images=None, pre_hms=None, hms=None,
    ...
    with torch.no_grad():
      ...
      with autocast(enabled=self.opt.withamp):
        ...
        if hms != None:
          hm_corr_hat, hm_pr_hat = self.model_second(images, pre_images, hms, pre_hms)

          # fusion stage
          #inp_fusion = torch.cat([output[0]['hm'], hm_corr_hat, hm_pr_hat], dim=1)
          #mas = self.model_fusion(inp_fusion)
          #mas = -torch.log(1. / mas - 1.)

          #pre_hms = (output[0]['hm'] + 1.) * hm_corr_hat - 1.
          pre_hms = output[0]['hm'] + hm_pr_hat
        
          #fuse output
          #pre_hms = output[0]['hm'] * (1 - mas) + ((output[0]['hm'] + 1.) * hm_corr_hat - 1.) * mas

          # additional direct frac corr or resi corr
          #pre_hms = output[0]['hm'] + hm_pr_hat


          #ma = torch.max(pre_hms.reshape(pre_hms.shape[0], -1), 1)[0]
          #mi = torch.min(pre_hms.reshape(pre_hms.shape[0], -1), 1)[0]
          #ma = ma.unsqueeze(1).unsqueeze(1).unsqueeze(1)
          #mi = mi.unsqueeze(1).unsqueeze(1).unsqueeze(1)
          #pre_hms = (pre_hms - mi) / (ma - mi)
          #pre_hms[pre_hms == 0] = 1e-4
          #pre_hms[pre_hms == 1] = 1 - 1e-4
