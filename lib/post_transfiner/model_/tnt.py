...
import math
from .refine_backbone import Backbone
...
from post_transfiner.position_encoding import build_position_encoding
...
class Block(nn.Module):
    ...
    def forward(self, inner_tokens, outer_tokens):
        if self.has_inner:
            ...
            outer_tokens[:,opt.n_p_q:] = outer_tokens[:,opt.n_p_q:] + self.proj_norm2(self.proj(self.proj_norm1(inner_tokens.reshape(B, N-opt.n_p_q, -1)))) # B, N, C
        if self.se > 0:
...
class Stem(nn.Module):
    ...
    def __init__(self, img_size=224, in_chans=4, outer_dim=768, inner_dim=24, position_embedding=None):
        super().__init__()
        ...
        self.inner_convs = nn.Sequential(
            nn.Conv2d(in_chans, inner_dim, 3, stride=1, padding=1),
            nn.BatchNorm2d(inner_dim),
            nn.ReLU(inplace=False),
        )
        self.outer_convs = nn.Sequential(
            nn.Conv2d(in_chans, inner_dim * 4, 3, stride=2, padding=1),
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        H_out, W_out = H // 4, W // 4  # patch (sentences)
        H_in, W_in = 4, 4  # words
        ...
        #mask = torch.zeros((outer_tokens.shape[0], outer_tokens.shape[2], outer_tokens.shape[3]), dtype=torch.bool,
         #                  device=outer_tokens.device)
        #outer_tokens_nest = NestedTensor(outer_tokens, mask)
        #pos_embed = self.position_embedding(outer_tokens_nest)
        ...
        inner_tokens = self.unfold(inner_tokens).transpose(1, 2) # + \
                      # pos_embed.permute(0, 2, 3, 1).reshape(B, H_out * W_out, -1) # B, N, Ck2
...
class TNT(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=64, num_classes=20, outer_dim=768, inner_dim=48,
                 ...
                 backbone=None, position_embedding=None):
        super().__init__()
        ...
        self.patch_embed = Stem(  # outer_tokens : bsz x num_patches x outer_dim    inner_tokens : (bsz x num_patches) x num_words x inner_dim
            img_size=img_size, in_chans=in_chans, outer_dim=outer_dim, inner_dim=inner_dim, position_embedding=position_embedding)
        ...
        self.cls_token = nn.Parameter(torch.zeros(1, opt.n_p_q, outer_dim))
        self.outer_tokens = nn.Parameter(torch.zeros(1, num_patches, outer_dim), requires_grad=False)
        self.outer_pos = nn.Parameter(torch.zeros(1, num_patches + opt.n_p_q, outer_dim))
        #self.cls_pos = nn.Parameter(torch.zeros(1, opt.n_p_q, outer_dim))
        ...
        self.norm = norm_layer(outer_dim) if not opt.aux_loss_tnt else nn.ModuleList([copy.deepcopy(norm_layer(outer_dim)) for _ in range(depth)])
        ...
        # Classifier head
        self.cls = nn.Linear(outer_dim, num_classes) if num_classes > 0 else nn.Identity()
        self.box = nn.Sequential(
            nn.Linear(outer_dim, outer_dim),
            nn.ReLU(),
            nn.Linear(outer_dim, outer_dim),
            nn.ReLU(),
            nn.Linear(outer_dim, 4)
        )
        self.cls = self.cls if not opt.aux_loss_tnt else nn.ModuleList([copy.deepcopy(self.cls) for _ in range(depth)])
        self.box = self.box if not opt.aux_loss_tnt else nn.ModuleList([copy.deepcopy(self.box) for _ in range(depth)])
    ...
    def forward_features(self, x):
        ...
        outer_tokens_list = [] if opt.aux_loss_tnt else None
        for blk in self.blocks:
            inner_tokens, outer_tokens = blk(inner_tokens, outer_tokens)
            if opt.aux_loss_tnt:
                outer_tokens_list.append(outer_tokens)

        outer_tokens = self.norm(outer_tokens) if not opt.aux_loss_tnt else \
            torch.stack([self.norm[i](outer_tokens_list[i]) for i in range(self.depth)], dim=0)
        return outer_tokens[:, 0:opt.n_p_q] if not opt.aux_loss_tnt else outer_tokens[:, :, 0:opt.n_p_q]
    ...
    def forward(self, x):
        ...
        cls = self.cls(x) if not opt.aux_loss_tnt else \
            torch.stack([self.cls[i](x[i]) for i in range(self.depth)], dim=0)

        # size: bsz x n_p_q x 4  in the form of 'center_x, center_y, w, h'
        box = self.box(x) if not opt.aux_loss_tnt else \
            torch.stack([self.box[i](x[i]).sigmoid() for i in range(self.depth)], dim=0)
...
def tnt_trans_patch16_192(pretrained=False, backbone=None, position_embedding=None, **kwargs):
    patch_size = 16
    inner_stride = 4
    outer_dim = 256
    inner_dim = 16
    ...
    outer_dim = make_divisible(outer_dim, outer_num_heads)
    inner_dim = make_divisible(inner_dim, inner_num_heads)
    model = TNT(img_size=(opt.input_h//4, opt.input_w//4), num_classes=num_cls, patch_size=patch_size, outer_dim=outer_dim,
                inner_dim=inner_dim, depth=opt.depth, outer_num_heads=outer_num_heads, inner_num_heads=inner_num_heads,
                qkv_bias=False, inner_stride=inner_stride, backbone=backbone, position_embedding=position_embedding, **kwargs)
    model.default_cfg = default_cfgs['tnt_s_patch16_224']
    if pretrained:
        load_pretrained(
            model, num_classes=model.num_classes, in_chans=kwargs.get('in_chans', 3), filter_fn=_conv_filter)
...
def build_transpatch_tnt(args):

    train_backbone = args.lr_backbone > 0
    backbone = DLASeg(34, heads={}, head_convs={}, opt=opt)
    position_embedding = build_position_encoding(args)
    transpatch = tnt_trans_patch16_192(False, backbone=backbone, position_embedding=position_embedding)
