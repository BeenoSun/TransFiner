...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=4, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.norm = self.norm if not opt.aux_loss_tnt else nn.ModuleList([copy.deepcopy(self.norm) for _ in range(depth)])
    ...
    def forward_features(self, x):
        ...
        x = x + temp_pos_embed
        x = self.pos_drop(x)
        x_list = [] if opt.aux_loss_tnt else None
        ...
        x = self.norm(x) if not opt.aux_loss_tnt else torch.stack([self.norm[i](x_list[i]) for i in range(self.depth)], dim=0)
