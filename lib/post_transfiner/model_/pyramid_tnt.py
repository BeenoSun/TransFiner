...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 2, 'input_size': (4, opt.input_h, opt.input_w), 'pool_size': None,
...
default_cfgs = {
    'tnt_trans_patch16_192': _cfg(
        input_size=(4, opt.input_h, opt.input_w), mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5),
    ),
...
class Block(nn.Module):
    ...
    def forward(self, x, outer_tokens, H_out, W_out, H_in, W_in, relative_pos): # inner_tokens, outer_tokens, H_out, W_out, H_in, W_in, self.relative_pos
...
class Stem(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        H_out, W_out = H // 8, W // 8 # patch (sentences)
        H_in, W_in = 4, 4 # words
...
class PyramidTNT(nn.Module):
    ...
    def __init__(self, configs=None, img_size=224, in_chans=3, num_classes=1000, mlp_ratio=4., qkv_bias=False,
        ...
        # Classifier head
        self.cls = nn.Linear(outer_dims[-1], num_classes) if num_classes > 0 else nn.Identity()
        self.box = nn.Sequential(
            nn.Linear(outer_dims[-1], outer_dims[-1]),
            nn.Linear(outer_dims[-1], outer_dims[-1]),
            nn.Linear(outer_dims[-1], 4),
        )
...
def ptnt_trans_patch16_192(pretrained=False, **kwargs):
    outer_dim = 80
    inner_dim = 5
    outer_head = 2
    inner_head = 1
    ...
    model = PyramidTNT(configs=configs, img_size=(opt.input_h, opt.input_w), in_chans=4, num_classes=1, qkv_bias=False, **kwargs)
    model.default_cfg = default_cfgs['tnt_trans_patch16_192']
    if pretrained:
        load_pretrained(
            model, num_classes=model.num_classes, in_chans=kwargs.get('in_chans', 3), filter_fn=_conv_filter)
    return model
...
def build_transpatch_ptnt(args):

    train_backbone = args.lr_backbone > 0
    transpatch = ptnt_trans_patch16_192(False)
