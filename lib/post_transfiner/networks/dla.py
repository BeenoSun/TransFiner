...
class DLA(nn.Module):
    def __init__(self, levels, channels, length, num_classes=1000,
        ...
        if length == 3:
            self.base_layer = nn.Sequential(
                nn.ConvTranspose2d(408, 128, 4, 2, 1, bias=False),
                nn.BatchNorm2d(128, momentum=BN_MOMENTUM),
                nn.ReLU(True),
                nn.ConvTranspose2d(128, 128, 4, 2, 1, bias=False),
                nn.BatchNorm2d(128, momentum=BN_MOMENTUM),
                nn.ReLU(True),
                nn.Conv2d(128, channels[0], kernel_size=7, stride=1,
...
def dla34(lenght, pretrained=True, **kwargs):  # DLA-34
    if lenght == 2:
        model = DLA([1, 1, 1, 2, 2, 1],
                    [64, 64, 64, 128, 256, 512], length=lenght,
                    block=BasicBlock, **kwargs)
    else:
        model = DLA([1, 1, 1, 2, 2, 1],
                    [16, 32, 64, 128, 256, 512], length=lenght,
                    block=BasicBlock, **kwargs)
    if pretrained:# or (lenght == 3):
        model_ori = timm.create_model('dla34', pretrained=True)
        pretrained_dict = model_ori.state_dict()
        model_dict = model.state_dict()
        # 1. filter out unnecessary keys
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if (k != 'base_layer.0.weight'
                                                                        and k != 'fc.weight'
                                                                        and k != 'fc.bias'
                                                                        and k in model_dict)}
...
class IDAUp(nn.Module):
    ...
    def forward(self, layers, startp, endp):
        for i in range(startp + 1, endp):
            ...
            layers[i] = layers[i].to(torch.float32)
...
class DLASeg(BaseModel):
    def __init__(self, num_layers, heads, head_convs, opt):
        ...
        self.last_level = 5
        self.base = globals()['dla{}'.format(num_layers)](lenght=len(heads),
    ...
    def img2feats(self, x):
        x = self.base(x)
        #x = [a.to(torch.float32) for a in x]
        x = self.dla_up(x)
        #x = [a.to(torch.float16) for a in x]
        y = []
