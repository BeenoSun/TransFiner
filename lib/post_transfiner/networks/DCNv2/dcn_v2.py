...
class _DCNv2(Function):
    ...
    def forward(
        ...
        output = output.to(torch.float16) if opt.withamp else output.to(torch.float32)
        return output
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = grad_input.to(torch.float16) if opt.withamp else grad_input.to(torch.float32)
        grad_offset = grad_offset.to(torch.float16) if opt.withamp else grad_offset.to(torch.float32)
        grad_mask = grad_mask.to(torch.float16) if opt.withamp else grad_mask.to(torch.float32)
